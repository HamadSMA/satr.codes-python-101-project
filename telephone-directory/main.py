...
#Defining a function that locates the key based on the value
def getkey(num):
...
#When "end" swtiches status, the loop will breal
while not end:
  ...
  #Case 2: The user is looking to find the name who's the phone number belongs to
        ...
        if getkey(direct_num) in directory:
  ...
  #Case 3: The user Will add an enrty to the phone directory
    ...
    answer = input(" ********** Type 'Y' to run again or anything else to quit ********** ").upper()
    if answer != "Y":
        end = True
